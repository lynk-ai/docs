...
- The cross-entity reference graph is **acyclic** — if a feature on entity A pulls a value across a relationship from entity B, and a definition on B pulls one back from A, neither entity can compile (the classic case: A's feature reads a metric on B that is itself derived from a metric on A). Break the loop by cutting **exactly one** back-edge — never both, or you lose a legitimate relationship — and sourcing that value from the entity's own columns.
