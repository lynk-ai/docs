...
**Go deeper** — [`deep/tool-output-shaping.md`](../deep/tool-output-shaping.md) has the four shapes for a large result and the truncation contract. Read it when designing a tool's return contract, debugging an agent that confidently misreports data, or trimming an agent loop's token bill.
