...
**Go deeper** — [`deep/hook-vs-router.md`](../deep/hook-vs-router.md) has where gates belong in a control flow and what fails open in practice. Read it when placing validation in an agent pipeline, designing gates/guardrails, or auditing which of your checks can silently not-fire.
