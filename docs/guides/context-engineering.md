...
- [Context reference](../context-reference/README.md) — how context behaves for the agent that reads it, and what to do about it. This guide sets the Lynk rules for what loads when; the reference holds the general craft underneath them and the measurements each rests on — wording something so it gets picked over its neighbours, when a file should become two, what a page costs every question that loads it, what will go stale or come to contradict something else. It applies to any layer content, not one file type, and while writing or reviewing it — not only to justify a rule here.
